# layouts/google_layout_legacy.py
# Version 10.01.01.14 dated 20260219
# Google Photos Legacy layout - Frozen stable reference of the original Google Photos-style layout
...
class GooglePhotosLayoutLegacy(BaseLayout):
    """
    ...
    def get_name(self) -> str:
        return "Google Photos Legacy"

    def get_id(self) -> str:
        return "google_legacy"
    ...
    def create_layout(self) -> QWidget:
        ...
        if self.project_id is None:
            ...
            if projects:
                ...
                print(f"[GooglePhotosLayoutLegacy] Using first project: {self.project_id}")
            else:
                print("[GooglePhotosLayoutLegacy] ⚠️ WARNING: No projects found! Please create a project first.")
        else:
            print(f"[GooglePhotosLayoutLegacy] Using default project: {self.project_id}")
        ...
        if getattr(self.main_window, '_deferred_init_started', False):
            ...
            if getattr(self, 'project_id', None):
                ...
                logger.info("[GooglePhotosLayoutLegacy] Suppressing initial load — no active project")
        else:
    ...
    def _on_search_state_changed(self, state):
        ...
        if not isValid(self.results_stack) or not isValid(self.empty_state) or not isValid(self.timeline):
            logger.debug("[GooglePhotosLayoutLegacy] Skipping SearchState update: UI objects already deleted")
            return
    ...
    def _on_startup_ready(self):
        ...
        self._startup_load_pending = False
        if not getattr(self, 'project_id', None):
            logger.info("[GooglePhotosLayoutLegacy] Startup ready but no project — suppressing load")
            return
        print("[GooglePhotosLayoutLegacy] First paint done — starting initial photo load")
        self._load_photos()
    ...
    def _create_timeline(self) -> QWidget:
        ...
        print("[GooglePhotosLayoutLegacy] ✅ Scroll-triggered lazy loading enabled")
    ...
    def _execute_coalesced_load(self):
        ...
        if sig == self._last_load_signature:
            print("[GooglePhotosLayoutLegacy] Skipping redundant reload (same state signature)")
            return
    ...
    def _load_photos(self, thumb_size: int = 200, filter_year: int = None, filter_month: int = None, filter_day: int = None, filter_folder: str = None, filter_person: str = None, filter_paths: list = None):
        ...
        print(f"[GooglePhotosLayoutLegacy] 📷 Loading photos from database (thumb size: {thumb_size}px){filter_str}...")
        ...
        print(f"[GooglePhotosLayoutLegacy] 🔍 Starting async photo load (generation {current_gen}) - existing content preserved...")
        ...
        if self.project_id is None:
            ...
            print("[GooglePhotosLayoutLegacy] ⚠️ No project selected")
            return
        ...
        if filter_paths is not None and len(filter_paths) == 0:
            ...
            print(f"[GooglePhotosLayoutLegacy] Empty path list → empty state (generation {current_gen})")
            return
        ...
        if filter_paths is not None:
            ...
            print(f"[GooglePhotosLayoutLegacy] Photo load worker started (generation {current_gen}, legacy/paths)")
        else:
            ...
            QThreadPool.globalInstance().start(worker)
            print(f"[GooglePhotosLayoutLegacy] Paged load started (generation {current_gen}, page_size={self._page_size})")
    ...
    def _show_refresh_indicator(self):
        ...
        try:
            ...
            logger.debug(f"[GooglePhotosLayoutLegacy] Could not show refresh indicator: {e}")
    ...
    def _clear_timeline_for_new_content(self):
        ...
        try:
            ...
            print(f"[GooglePhotosLayoutLegacy] ⚠️ Error clearing timeline: {e}")
    ...
    def _group_photos_by_date(self, rows) -> Dict[str, List[Tuple]]:
        ...
        for row in rows:
            ...
            if date_taken:  # Should always be true since created_date is never NULL
                ...
                print(f"[GooglePhotosLayoutLegacy] ⚠️ WARNING: Photo has no created_date: {path}")
    ...
    def _build_timeline_tree(self, photos_by_date: Dict[str, List[Tuple]]):
        ...
        for date_str in photos_by_date.keys():
            ...
            try:
                ...
                print(f"[GooglePhotosLayoutLegacy] ⚠️ Failed to parse date '{date_str}': {e}")
                continue
    ...
    def _on_timeline_item_clicked(self, item: QTreeWidgetItem, column: int):
        ...
        if item_type == "year":
            year = data.get("year")
            print(f"[GooglePhotosLayoutLegacy] Filtering by year: {year}")
            self._load_photos(
            ...
            print(f"[GooglePhotosLayoutLegacy] Filtering by month: {month_name}")
            self._load_photos(
                thumb_size=self.current_thumb_size,
                filter_year=year,
                filter_month=month,
    ...
    def _on_folder_item_clicked(self, item: QTreeWidgetItem, column: int):
        ...
        if folder_path:
            ...
            print(f"[GooglePhotosLayoutLegacy] Filtering by folder: {folder_name}")
            self._load_photos(
                thumb_size=self.current_thumb_size,
                filter_year=None,
                filter_month=None,
    ...
    def _build_tags_tree(self):
        ...
        try:
            ...
            print(f"[GooglePhotosLayoutLegacy] ⚠️ Error loading tags: {e}")
            import traceback
            traceback.print_exc()
            return
        ...
        print(f"[GooglePhotosLayoutLegacy] ✓ Built tags tree: {len(tag_rows)} tags")
    ...
    def _build_people_tree(self):
        ...
        print("[GooglePhotosLayoutLegacy] 🔍 _build_people_tree() called")
        try:
            ...
            print(f"[GooglePhotosLayoutLegacy] 👥 Querying face_branch_reps for project_id={self.project_id}")
            ...
            print(f"[GooglePhotosLayoutLegacy] 👥 Found {len(rows)} face clusters in database")
            ...
            if hasattr(self, 'people_section'):
                ...
                print(f"[GooglePhotosLayoutLegacy] ✓ Updated people section count badge: {total_photos}")
            else:
                print("[GooglePhotosLayoutLegacy] ⚠️ people_section not found!")

            # Clear existing grid
            if hasattr(self, 'people_grid'):
                self.people_grid.clear()
                print(f"[GooglePhotosLayoutLegacy] ✓ Cleared people grid")
            else:
                print("[GooglePhotosLayoutLegacy] ⚠️ people_grid not found!")
                return

            if not rows:
                # No face clusters found
                print("[GooglePhotosLayoutLegacy] No faces found - grid will show empty state")
            ...
            for branch_key, label, count, rep_path, rep_thumb_png in rows:
                ...
                if rep_thumb_png:
                    try:
                        ...
                        if success:
                            print(f"[GooglePhotosLayoutLegacy]   ✓ Loaded pixmap for {display_name}: {face_pixmap.width()}x{face_pixmap.height()}")
                        else:
                            print(f"[GooglePhotosLayoutLegacy]   ⚠️ Failed to load pixmap for {display_name}")
                            face_pixmap = None
                    except Exception as e:
                        print(f"[GooglePhotosLayoutLegacy] ⚠️ Error loading face pixmap for {display_name}: {e}")
                        face_pixmap = None
                    ...
                    try:
                        ...
                        if os.path.exists(rep_path):
                            ...
                            if not file_pixmap.isNull():
                                ...
                                print(f"[GooglePhotosLayoutLegacy]   ✓ Loaded pixmap from file for {display_name}")
                            else:
                                print(f"[GooglePhotosLayoutLegacy]   ⚠️ Pixmap from file is null for {display_name}")
                        else:
                            print(f"[GooglePhotosLayoutLegacy]   ⚠️ rep_path not found for {display_name}: {rep_path}")
                    except Exception as e:
                        print(f"[GooglePhotosLayoutLegacy] ⚠️ Error loading face pixmap from file for {display_name}: {e}")

                # Add to grid with both branch_key and display_name
                if hasattr(self, 'people_grid'):
                    self.people_grid.add_person(branch_key, display_name, face_pixmap, count)
                    added_count += 1
                    print(f"[GooglePhotosLayoutLegacy]   ✓ Added to grid [{added_count}/{len(rows)}]: {display_name} ({count} photos)")

            print(f"[GooglePhotosLayoutLegacy] ✅ Populated people grid with {added_count} faces")
            ...
            print(f"[GooglePhotosLayoutLegacy] ✅ _build_people_tree() completed successfully")

        except Exception as e:
            print(f"[GooglePhotosLayoutLegacy] ⚠️ Error building people grid: {e}")
            import traceback
            traceback.print_exc()
    ...
    def _on_accordion_date_clicked(self, date_key: str):
        ...
        """
        print(f"[GooglePhotosLayoutLegacy] Accordion date clicked: {date_key}")
    ...
    def _on_accordion_folder_clicked(self, folder_id: int):
        ...
        if self._pending_folder_id == folder_id:
            print(f"[GooglePhotosLayoutLegacy] Folder {folder_id} already pending, skipping")
            return

        print(f"[GooglePhotosLayoutLegacy] ========================================")
        print(f"[GooglePhotosLayoutLegacy] Accordion folder clicked: folder_id={folder_id} (debouncing...)")
    ...
    def _execute_folder_click(self):
        ...
        print(f"[GooglePhotosLayoutLegacy] Executing debounced folder click: folder_id={folder_id}")
        ...
        try:
            ...
            with db._connect() as conn:
                ...
                if row:
                    folder_path = row[0]
                    print(f"[GooglePhotosLayoutLegacy] Found folder path: {folder_path}")
                    print(f"[GooglePhotosLayoutLegacy] Calling _load_photos with filter_folder={folder_path}")
                    self._load_photos(
                        thumb_size=self.current_thumb_size,
                        filter_year=None,
                        filter_month=None,
                    ...
                    print(f"[GooglePhotosLayoutLegacy] _load_photos call completed")
                else:
                    print(f"[GooglePhotosLayoutLegacy] ERROR: No folder found with id={folder_id}")
        except Exception as e:
            print(f"[GooglePhotosLayoutLegacy] ERROR loading folder: {e}")
            import traceback
            traceback.print_exc()
        print(f"[GooglePhotosLayoutLegacy] ========================================")
    ...
    def _on_accordion_tag_clicked(self, tag_name: str):
        ...
        """
        print(f"[GooglePhotosLayoutLegacy] Accordion tag clicked: {tag_name}")
        self._filter_by_tag(tag_name)
    ...
    def _on_accordion_branch_clicked(self, branch_key: str):
        ...
        """
        print(f"[GooglePhotosLayoutLegacy] Accordion branch clicked: {branch_key}")
    ...
    def _on_accordion_person_clicked(self, person_branch_key: str):
        ...
        if person_branch_key == "":
            logger.info("[GooglePhotosLayoutLegacy] Clearing person filter from accordion toggle")
            self._request_load(
                thumb_size=self.current_thumb_size,
        ...
        logger.info(
            "[GooglePhotosLayoutLegacy] Accordion person clicked: %s", person_branch_key
        )
    ...
    def _on_accordion_group_clicked(self, group_id: int, match_mode: str = "together"):
        ...
        if group_id is None or group_id < 1:
            logger.info(f"[GooglePhotosLayoutLegacy] Group deselected or invalid (group_id={group_id})")
        ...
        logger.info(f"[GooglePhotosLayoutLegacy] Group clicked: {group_id} (mode={match_mode})")
        ...
        try:
            ...
            logger.info(f"[GooglePhotosLayoutLegacy] Group {group_id} has {len(paths)} matching photos")
            ...
            logger.error(f"[GooglePhotosLayoutLegacy] Failed to load group photos: {e}", exc_info=True)

    def _on_group_created(self):
        """Handle new group creation - refresh sidebar groups."""
        logger.info("[GooglePhotosLayoutLegacy] Group created, refreshing People section")
    ...
    def _on_group_edit_requested(self, group_id: int):
        ...
        """
        logger.info(f"[GooglePhotosLayoutLegacy] Edit group requested: {group_id}")
        ...
        try:
            ...
            if dialog.exec():
                ...
                if hasattr(self, "accordion_sidebar") and hasattr(self.accordion_sidebar, '_compute_group_matches'):
                    ...
                    self._refresh_people_sidebar()
                logger.info(f"[GooglePhotosLayoutLegacy] Group {group_id} edited, recomputing matches")
        except Exception as e:
            logger.error(f"[GooglePhotosLayoutLegacy] Failed to open edit group dialog: {e}", exc_info=True)


    def _on_group_deleted(self, group_id: int):
        logger.info(f"[GooglePhotosLayoutLegacy] Group deleted: {group_id}")
    ...
    def _on_accordion_location_clicked(self, location_data: dict):
        ...
        """
        logger.info(
            "[GooglePhotosLayoutLegacy] Accordion location clicked: %s (%d photos)",
        ...
        if not paths:
            logger.warning("[GooglePhotosLayoutLegacy] Location has no photos")
            return
    ...
    def _on_accordion_person_merged(self, source_branch: str, target_branch: str):
        ...
        logger.info(
            "[GooglePhotosLayoutLegacy] Person merge detected (%s -> %s); refreshing grid",
            source_branch,
            target_branch,
        )
    ...
    def _refresh_people_sidebar(self):
        try:
            ...
            logger.debug("[GooglePhotosLayoutLegacy] Failed to refresh people section after tool action: %s", e)
    ...
    def _on_people_merge_history_requested(self):
        try:
            ...
            logger.debug("[GooglePhotosLayoutLegacy] Merge history request failed: %s", e, exc_info=True)
    ...
    def _on_people_undo_requested(self):
        try:
            ...
            logger.debug("[GooglePhotosLayoutLegacy] Undo merge request failed: %s", e, exc_info=True)
    ...
    def _on_people_redo_requested(self):
        try:
            ...
            logger.debug("[GooglePhotosLayoutLegacy] Redo merge request failed: %s", e, exc_info=True)
    ...
    def _on_people_tools_requested(self):
        ...
        try:
            ...
            logger.debug("[GooglePhotosLayoutLegacy] People tools request failed: %s", e, exc_info=True)
    ...
    def _open_face_quality_dashboard(self):
        ...
        try:
            ...
            dashboard.show()
            logger.info("[GooglePhotosLayoutLegacy] Opened Face Quality Dashboard")

        except Exception as e:
            logger.error(f"[GooglePhotosLayoutLegacy] Failed to open Face Quality Dashboard: {e}")
    ...
    def _open_manual_face_crop_selector(self):
        ...
        try:
            ...
            logger.error(f"[GooglePhotosLayoutLegacy] Failed to open photo selector: {e}")
            QMessageBox.critical(
                self.main_window if hasattr(self, 'main_window') else None,
                "Error",
    ...
    def _open_manual_face_crop_editor(self, photo_path: str):
        ...
        try:
            logger.info(f"[GooglePhotosLayoutLegacy] Preparing to open Face Crop Editor for: {photo_path}")

            from ui.face_crop_editor import FaceCropEditor
            logger.info(f"[GooglePhotosLayoutLegacy] ✓ FaceCropEditor imported")

            if not os.path.exists(photo_path):
                logger.warning(f"[GooglePhotosLayoutLegacy] Photo not found: {photo_path}")
                QMessageBox.warning(
                    self.main_window if hasattr(self, 'main_window') else None,
                    "Photo Not Found",
                    f"Photo not found:\n{photo_path}"
            ...
            logger.info(f"[GooglePhotosLayoutLegacy] Creating FaceCropEditor instance...")
            logger.info(f"[GooglePhotosLayoutLegacy]   - photo_path: {photo_path}")
            logger.info(f"[GooglePhotosLayoutLegacy]   - project_id: {self.project_id}")
            logger.info(f"[GooglePhotosLayoutLegacy]   - parent: {self.main_window if hasattr(self, 'main_window') else None}")
            ...
            logger.info(f"[GooglePhotosLayoutLegacy] ✓ FaceCropEditor instance created successfully")
            ...
            logger.info(f"[GooglePhotosLayoutLegacy] Showing Face Crop Editor dialog...")
            ...
            try:
                ...
                logger.info(f"[GooglePhotosLayoutLegacy] Face Crop Editor closed (result={result}, faces_saved={faces_were_saved})")
            except RuntimeError as e:
                # Dialog object already deleted - this is the crash we're trying to avoid!
                logger.warning(f"[GooglePhotosLayoutLegacy] Could not access editor.faces_were_saved: {e}")
                logger.warning(f"[GooglePhotosLayoutLegacy] Dialog was deleted too quickly - assuming no faces saved")

            # Now use the LOCALLY STORED flag to decide if we should refresh
            # This happens AFTER editor object is safely deleted
            if faces_were_saved:
                logger.info(f"[GooglePhotosLayoutLegacy] Manual faces were saved, scheduling People section refresh...")
                ...
                logger.info(f"[GooglePhotosLayoutLegacy] ✓ People section refresh scheduled (delayed 100ms)")

        except Exception as e:
            logger.error(f"[GooglePhotosLayoutLegacy] Failed to open Face Crop Editor: {e}", exc_info=True)
            QMessageBox.critical(
                self.main_window if hasattr(self, 'main_window') else None,
                "Error",
    ...
    def _refresh_people_sidebar_after_face_save(self):
        ...
        """
        try:
            logger.info("[GooglePhotosLayoutLegacy] Executing delayed People section refresh...")
            if hasattr(self, "accordion_sidebar"):
                self.accordion_sidebar.reload_people_section()
                logger.info("[GooglePhotosLayoutLegacy] ✓ People section refreshed successfully after manual face save")
            else:
                logger.warning("[GooglePhotosLayoutLegacy] No accordion_sidebar found - cannot refresh People section")
        except RuntimeError as e:
            # Qt object might still be deleted - log but don't crash
            logger.error(f"[GooglePhotosLayoutLegacy] Qt object deleted during People refresh: {e}", exc_info=True)
            logger.error(f"[GooglePhotosLayoutLegacy] This indicates a Qt lifecycle bug - please report")
        except Exception as e:
            logger.error(f"[GooglePhotosLayoutLegacy] Failed to refresh People section: {e}", exc_info=True)
    ...
    def _on_accordion_person_deleted(self, branch_key: str):
        ...
        logger.info(
            "[GooglePhotosLayoutLegacy] Active person '%s' deleted; clearing filter", branch_key
        )
    ...
    def _on_accordion_video_clicked(self, filter_spec: str):
        ...
        """
        print(f"[GooglePhotosLayoutLegacy] Accordion video clicked: {filter_spec}")
                ...
                if filter_type == "duration":
                    ...
                    with db._connect() as conn:
                        ...
                        if video_rows:
                            for row in video_rows[:5]:  # Log first 5 for debugging
                                ...
                                print(f"[GooglePhotosLayoutLegacy] 📐 {filter_value.upper()}: {fname} ({w}x{h}, max={max_dim})")
                            if len(video_rows) > 5:
                                print(f"[GooglePhotosLayoutLegacy] 📐 ... and {len(video_rows) - 5} more {filter_value.upper()} videos")
                    ...
                    with db._connect() as conn:
                        ...
                        try:
                            ...
                            print(f"[GooglePhotosLayoutLegacy] ⚠️ Invalid year filter: {filter_value}")

            # Rebuild timeline with video results
            self._rebuild_timeline_with_results(video_rows, f"Videos: {filter_spec}")

        except Exception as e:
            print(f"[GooglePhotosLayoutLegacy] ⚠️ Error filtering videos: {e}")
            import traceback
            traceback.print_exc()
    ...
    def _filter_by_tag(self, tag_name: str):
        ...
        try:
            ...
            print(f"[GooglePhotosLayoutLegacy] ⚠️ Error filtering by tag '{tag_name}': {e}")
        ...
        try:
            ...
            if rep_thumb_png:
                try:
                    ...
                    print(f"[GooglePhotosLayoutLegacy] Failed to load thumbnail from BLOB: {blob_error}")
            ...
            if rep_path and os.path.exists(rep_path):
                try:
                    ...
                    print(f"[GooglePhotosLayoutLegacy] Failed to load thumbnail from {rep_path}: {file_error}")

            return None

        except Exception as e:
            print(f"[GooglePhotosLayoutLegacy] Error loading face thumbnail: {e}")
            return None
    ...
    def _on_people_item_clicked(self, item: QTreeWidgetItem, column: int):
        ...
        if branch_key:
            ...
            print(f"[GooglePhotosLayoutLegacy] Filtering by person: {label} (branch_key={branch_key})")
            self._load_photos(
                thumb_size=self.current_thumb_size,
                filter_year=None,
                filter_month=None,
    ...
    def _on_person_clicked_from_grid(self, person_name: str):
        ...
        """
        print(f"[GooglePhotosLayoutLegacy] Filtering by person from grid: {person_name}")
        self._load_photos(
            thumb_size=self.current_thumb_size,
            filter_year=None,
            filter_month=None,
    ...
    def _prompt_quick_name_dialog(self):
        ...
        try:
            ...
            print(f"[GooglePhotosLayoutLegacy] Quick name dialog failed: {e}")
    ...
    def _prompt_bulk_face_review(self):
        ...
        try:
            ...
            print(f"[GooglePhotosLayoutLegacy] Bulk review dialog failed: {e}")
    ...
    def _suggest_cluster_merges(self, named_clusters, similarity_threshold=0.75):
        ...
        try:
            ...
            print(f"[GooglePhotosLayoutLegacy] Merge suggestion calculation failed: {e}")
            return {}
    ...
    def _show_merge_suggestions_dialog(self, suggestions, parent=None):
        ...
        try:
            ...
            print(f"[GooglePhotosLayoutLegacy] Merge suggestions dialog failed: {e}")
    def _on_person_context_menu(self, branch_key: str, action: str):
        """Handle context menu action on person card."""
        print(f"[GooglePhotosLayoutLegacy] Context menu action '{action}' for {branch_key}")
        ...
        try:
            ...
            print(f"[GooglePhotosLayoutLegacy] Error fetching person name: {e}")
    ...
    def _filter_people_grid(self, text: str):
        ...
        print(f"[GooglePhotosLayoutLegacy] Filtering people grid: '{search_query}'")
        ...
        if hasattr(self, 'people_grid') and hasattr(self.people_grid, 'flow_layout'):
            ...
            print(f"[GooglePhotosLayoutLegacy] Filter results: {visible_count} people visible")
    ...
    def _on_people_search_OLD_REMOVED(self, text: str):
        ...
        """
        search_query = text.lower().strip()
        print(f"[GooglePhotosLayoutLegacy] Searching people: '{search_query}'")
        ...
        if hasattr(self, 'people_grid') and hasattr(self.people_grid, 'flow_layout'):
            ...
            print(f"[GooglePhotosLayoutLegacy] Search results: {visible_count} people visible")
    ...
    def _rename_person(self, item: QTreeWidgetItem, branch_key: str, current_name: str):
        ...
        try:
            ...
            print(f"[GooglePhotosLayoutLegacy] Person renamed: {current_name} → {new_name}")
            QMessageBox.information(self.main_window, "Renamed", f"Person renamed to '{new_name}'")

        except Exception as e:
            print(f"[GooglePhotosLayoutLegacy] Rename failed: {e}")
    ...
    def _perform_merge(self, source_key: str, target_key: str, source_name: str):
        ...
        try:
            ...
            print(f"[GooglePhotosLayoutLegacy] Merge successful: {source_name} merged into {target_key}")
            print(f"[GooglePhotosLayoutLegacy] Merge result: {result}")
            ...
            print(f"[GooglePhotosLayoutLegacy] Merge failed: {e}")
            import traceback
            traceback.print_exc()
    ...
    def _on_drag_merge(self, source_branch: str, target_branch: str):
        ...
        try:
            ...
            print(f"[GooglePhotosLayoutLegacy] Drag-drop merge failed: {e}")
            import traceback
            traceback.print_exc()
    ...
    def _undo_last_merge(self):
        ...
        try:
            ...
            if result:
                ...
                print(f"[GooglePhotosLayoutLegacy] Undo successful: {result}")
            else:
                QMessageBox.information(
                    self.main_window,
                    "No Undo Available",
            ...
            print(f"[GooglePhotosLayoutLegacy] Undo failed: {e}")
            import traceback
            traceback.print_exc()
    ...
    def _redo_last_undo(self):
        ...
        try:
            ...
            if sources:
                ...
                print(f"[GooglePhotosLayoutLegacy] Redo successful: {result}")
            
        except Exception as e:
            print(f"[GooglePhotosLayoutLegacy] Redo failed: {e}")
            import traceback
            traceback.print_exc()
    ...
    def _update_undo_redo_state(self):
        ...
        try:
            ...
            print(f"[GooglePhotosLayoutLegacy] Failed to update undo/redo buttons: {e}")
    ...
    def _show_merge_history(self):
        ...
        try:
            ...
            print(f"[GooglePhotosLayoutLegacy] Failed to show merge history: {e}")
            import traceback
            traceback.print_exc()
    ...
    def _prompt_merge_suggestions(self, target_branch_key: str):
        ...
        try:
            ...
            try:
                ...
                print(f"[GooglePhotosLayoutLegacy] Failed to load target preview: {e}")
            ...
            try:
                ...
                print(f"[GooglePhotosLayoutLegacy] Failed to load merge history: {e}")
            ...
            for i, (bk, name, cnt, sim, rep_path, rep_thumb) in enumerate(suggestions):
                ...
                try:
                    ...
                    face_label.setStyleSheet("background: #e8eaed; border-radius: 40px; font-size: 24pt;")
                    face_label.setText("👤")
                    print(f"[GooglePhotosLayoutLegacy] Failed to load suggestion preview: {e}")
            ...
            print(f"[GooglePhotosLayoutLegacy] Merge suggestions failed: {e}")
            import traceback
            traceback.print_exc()
        ...
        try:
            ...
            def remove_selected():
                ...
                with ReferenceDB()._connect() as conn:
                    ...
                    conn.commit()
                    print(f"[GooglePhotosLayoutLegacy] Removed {len(ids)} faces from {source_branch} to {target}, counts updated")
            ...
            def merge_selected():
                ...
                if picker_dlg.exec() == QDialog.Accepted:
                    ...
                    with ReferenceDB()._connect() as conn:
                        ...
                        conn.commit()
                        print(f"[GooglePhotosLayoutLegacy] Merged {len(ids)} faces from {source_branch} to {selected_target}, counts updated")
            ...
            print(f"[GooglePhotosLayoutLegacy] Person detail failed: {e}")
    ...
    def _delete_person(self, branch_key: str, person_name: str):
        ...
        try:
            ...
            print(f"[GooglePhotosLayoutLegacy] Person deleted: {person_name}")
            QMessageBox.information(self.main_window, "Deleted", f"'{person_name}' deleted successfully")

        except Exception as e:
            print(f"[GooglePhotosLayoutLegacy] Delete failed: {e}")
    ...
    def _on_section_header_clicked(self):
        ...
        """
        print("[GooglePhotosLayoutLegacy] Section header clicked - clearing all filters")
    ...
    def _toggle_date_group(self, date_str: str, collapse_btn: QPushButton):
        ...
        try:
            ...
            if not grid:
                print(f"[GooglePhotosLayoutLegacy] ⚠️ Grid not found for {date_str}")
                return
            ...
            if new_state:  # Collapsing
                ...
                print(f"[GooglePhotosLayoutLegacy] ▲ Collapsed date group: {date_str}")
            else:  # Expanding
                grid.show()
                collapse_btn.setText("▼")
                print(f"[GooglePhotosLayoutLegacy] ▼ Expanded date group: {date_str}")

        except Exception as e:
            print(f"[GooglePhotosLayoutLegacy] ⚠️ Error toggling date group {date_str}: {e}")
    ...
    def _calculate_responsive_columns(self, thumb_size: int) -> int:
        ...
        if not hasattr(self, '_last_column_count') or self._last_column_count != cols:
            print(f"[GooglePhotosLayoutLegacy] 📐 Responsive grid: {cols} columns (viewport: {viewport_width}px, thumb: {thumb_size}px)")
    ...
    def _on_thumbnail_loaded(self, path: str, qimage, size: int):
        ...
        try:
            ...
            print(f"[GooglePhotosLayoutLegacy] Error updating thumbnail for {path}: {e}")
            button.setText("❌")
    ...
    def _display_photos_in_timeline(self, rows: list):
        ...
        if not rows:
            ...
            self.timeline_layout.addWidget(empty_widget)
            print(f"[GooglePhotosLayoutLegacy] No photos found in project {self.project_id}")
            return

        print(f"[GooglePhotosLayoutLegacy] Dispatching {len(rows)} assets (photos+videos) to background grouping worker...")
    ...
    def _on_grouping_done(self, generation: int, photos_by_date: dict, rows: list):
        ...
        if generation != self._photo_load_generation:
            print(f"[GooglePhotosLayoutLegacy] Discarding stale grouping result (gen {generation})")
            return

        print(f"[GooglePhotosLayoutLegacy] Grouped into {len(photos_by_date)} date groups")
        ...
        print(f"[GooglePhotosLayoutLegacy] Tracking {len(self.all_displayed_paths)} paths for multi-selection")
    ...
    def _finalize_timeline_display(self):
        ...
        if self.virtual_scroll_enabled:
            print(f"[GooglePhotosLayoutLegacy] Virtual scrolling enabled: {len(photos_by_date)} total date groups")
            print(f"[GooglePhotosLayoutLegacy] Rendered: {len(self.rendered_date_groups)} groups | Placeholders: {len(photos_by_date) - len(self.rendered_date_groups)} groups")
        else:
            print(f"[GooglePhotosLayoutLegacy] Loaded {len(rows)} photos in {len(photos_by_date)} date groups")

        print(f"[GooglePhotosLayoutLegacy] Queued {self.thumbnail_load_count} thumbnails for loading (initial limit: {self.initial_load_limit})")
        print(f"[GooglePhotosLayoutLegacy] Photo loading complete! Thumbnails will load progressively.")
    ...
    def _on_scroll_debounced(self):
        ...
        if paths_to_load:
            ...
            if actually_queued > 0:
                print(f"[GooglePhotosLayoutLegacy] 📜 Queued {actually_queued} thumbnails, {len(self.unloaded_thumbnails)} remaining")
    ...
    def _create_thumbnail(self, path: str, size: int) -> QWidget:
        ...
        if self.thumbnail_aspect_ratio == "square":
            ...
            try:
                ...
                print(f"[GooglePhotosLayoutLegacy] Warning: Could not read image dimensions for {os.path.basename(path)}: {e}")
                container_width = size
                container_height = size
        ...
        try:
            ...
            print(f"[GooglePhotosLayoutLegacy] Warning: Could not load tags for {os.path.basename(path)}: {e}")
    ...
    def _on_photo_clicked(self, path: str):
        ...
        print(f"[GooglePhotosLayoutLegacy] Photo clicked: {path}")
        ...
        if shift_pressed and self.last_selected_path and self.all_displayed_paths:
            print(f"[GooglePhotosLayoutLegacy] Shift+Click range selection from {self.last_selected_path} to {path}")
            try:
                ...
                print(f"[GooglePhotosLayoutLegacy] ✓ Range selected: {end_idx - start_idx + 1} photos")
                return

            except (ValueError, IndexError) as e:
                print(f"[GooglePhotosLayoutLegacy] ⚠️ Range selection error: {e}")
                # Fall through to normal selection

        # CTRL+CLICK: Toggle selection (add/remove)
        if ctrl_pressed:
            print(f"[GooglePhotosLayoutLegacy] Ctrl+Click toggle selection: {path}")
    ...
    def _open_photo_lightbox(self, path: str):
        ...
        """
        print(f"[GooglePhotosLayoutLegacy] 👁️ Opening lightbox for: {path}")

        # Collect all media paths (photos + videos) in timeline order
        all_media = self._get_all_media_paths()

        if not all_media:
            print("[GooglePhotosLayoutLegacy] ⚠️ No media to display in lightbox")
            return
        ...
        try:
            ...
            lightbox.exec()
            print("[GooglePhotosLayoutLegacy] MediaLightbox closed")
            ...
            print(f"[GooglePhotosLayoutLegacy] ⚠️ Error opening lightbox: {e}")
            import traceback
            traceback.print_exc()
    ...
    def _get_all_media_paths(self) -> List[str]:
        ...
        try:
            ...
            with db._connect() as conn:
                ...
                print(f"[GooglePhotosLayoutLegacy] Found {len(photo_paths)} photos + {len(video_paths)} videos = {len(all_paths)} total media")

        except Exception as e:
            print(f"[GooglePhotosLayoutLegacy] ⚠️ Error fetching media paths: {e}")
    ...
    def _update_selection_ui(self):
        ...
        print(f"[GooglePhotosLayoutLegacy] Selection updated: {count} photos selected")
    ...
    def _on_select_all(self):
        ...
        self._update_selection_ui()
        print(f"[GooglePhotosLayoutLegacy] ✓ Selected all {len(self.selected_photos)} photos")
    ...
    def _on_clear_selection(self):
        ...
        self._update_selection_ui()
        print("[GooglePhotosLayoutLegacy] ✗ Cleared all selections")
    ...
    def _on_copy_gps_from_toolbar(self):
        ...
        print(f"[GooglePhotosLayoutLegacy] 📍 Copied GPS from {os.path.basename(first_photo)} via toolbar")
    ...
    def _on_invert_selection(self):
        ...
        if not self.all_displayed_paths:
            print("[GooglePhotosLayoutLegacy] ⚠️ No photos to invert selection")
            return
        ...
        self._update_selection_ui()
        print(f"[GooglePhotosLayoutLegacy] ⇄ Inverted selection: {len(new_selection)} photos now selected")
    ...
    def _show_photo_context_menu(self, path: str, global_pos):
        ...
        try:
            ...
            if chosen is act_remove_all_tags:
                ...
                for tag_name in list(current_tags):
                    try:
                        ...
                        print(f"[GooglePhotosLayoutLegacy] ⚠️ Failed to remove tag '{tag_name}': {e}")
            ...
            print(f"[GooglePhotosLayoutLegacy] ⚠️ Context menu error: {e}")
            import traceback
            traceback.print_exc()
    ...
    def _show_in_explorer(self, path: str):
        ...
        try:
            ...
            print(f"[GooglePhotosLayoutLegacy] 📁 Opened location: {path}")
        except Exception as e:
            print(f"[GooglePhotosLayoutLegacy] ⚠️ Error opening location: {e}")
    ...
    def _copy_path_to_clipboard(self, path: str):
        ...
        print(f"[GooglePhotosLayoutLegacy] 📋 Copied to clipboard: {path}")

    def _edit_photo_location(self, path: str):
        """Edit GPS location for a photo (manual location editing)."""
        from PySide6.QtWidgets import QMessageBox

        print(f"[GooglePhotosLayoutLegacy] 📍 Opening location editor for: {path}")
        ...
        try:
            ...
            if location_changed:
                print(f"[GooglePhotosLayoutLegacy] ✓ Location updated for {os.path.basename(path)}")

                # Reload Locations section in accordion sidebar
                try:
                    if hasattr(self, 'accordion_sidebar'):
                        print("[GooglePhotosLayoutLegacy] Reloading Locations section...")
                        self.accordion_sidebar.reload_section("locations")
                    else:
                        print("[GooglePhotosLayoutLegacy] Warning: No accordion_sidebar reference")
                except Exception as e:
                    print(f"[GooglePhotosLayoutLegacy] Warning: Failed to reload Locations section: {e}")
            ...
            print(f"[GooglePhotosLayoutLegacy] Error opening location editor: {e}")
            import traceback
            traceback.print_exc()
            QMessageBox.critical(
                self.main_window,
    ...
    def _edit_photos_location_batch(self, paths: list[str]):
        ...
        print(f"[GooglePhotosLayoutLegacy] 📍 Opening batch location editor for {len(paths)} photos")
        ...
        if confirm != QMessageBox.Yes:
            print("[GooglePhotosLayoutLegacy] Batch location edit cancelled by user")
            return
        ...
        try:
            ...
            # If location was changed, refresh the Locations section
            if location_changed:
                print(f"[GooglePhotosLayoutLegacy] ✓ Location updated for {len(paths)} photos")
            ...
            print(f"[GooglePhotosLayoutLegacy] Error opening batch location editor: {e}")
            import traceback
            traceback.print_exc()
            QMessageBox.critical(
                self.main_window,
    ...
    def _copy_location(self, path: str):
        ...
        try:
            ...
            print(f"[GooglePhotosLayoutLegacy] ✓ Copied GPS location: {location_display} ({lat:.6f}, {lon:.6f})")

        except Exception as e:
            print(f"[GooglePhotosLayoutLegacy] Error copying location: {e}")
            import traceback
            traceback.print_exc()
            QMessageBox.critical(
                self.main_window,
        ...
        try:
            ...
            for photo_path in paths:
                try:
                    ...
                    success_count += 1
                    print(f"[GooglePhotosLayoutLegacy] ✓ Pasted location to: {os.path.basename(photo_path)}")

                except Exception as e:
                    fail_count += 1
                    print(f"[GooglePhotosLayoutLegacy] ✗ Failed to paste location to {os.path.basename(photo_path)}: {e}")
            ...
            print(f"[GooglePhotosLayoutLegacy] ✓ Paste complete: {success_count} success, {fail_count} failures")

        except Exception as e:
            print(f"[GooglePhotosLayoutLegacy] Error pasting location: {e}")
            import traceback
            traceback.print_exc()
            QMessageBox.critical(
                self.main_window,
...
Modified: {datetime.fromtimestamp(stat.st_mtime).strftime('%Y-%m-%d %H:%M:%S')}
            ...
            print(f"[GooglePhotosLayoutLegacy] ℹ️ Showing properties: {path}")
        except Exception as e:
            QMessageBox.warning(None, "Error", f"Could not load properties:\n{e}")
            print(f"[GooglePhotosLayoutLegacy] ⚠️ Error showing properties: {e}")
    ...
    def keyPressEvent(self, event: QKeyEvent):
        ...
        if key == Qt.Key_A and modifiers == Qt.ControlModifier:
            print("[GooglePhotosLayoutLegacy] ⌨️ Ctrl+A - Select all")
            self._on_select_all()
            event.accept()
        ...
        # Ctrl+D: Deselect All
            ...
            if len(self.selected_photos) > 0:
                print("[GooglePhotosLayoutLegacy] ⌨️ Ctrl+D - Deselect all")
        ...
        # Escape: Clear selection/filter
            ...
            if len(self.selected_photos) > 0:
                print("[GooglePhotosLayoutLegacy] ⌨️ ESC - Clear selection")
                self._on_clear_selection()
                event.accept()
            elif hasattr(self, 'active_person_filter') and self.active_person_filter:
                print("[GooglePhotosLayoutLegacy] ⌨️ ESC - Clear person filter")
                self._clear_filter()
                event.accept()
            else:
                super().keyPressEvent(event)
        ...
        # Delete: Delete selected photos
            ...
            if len(self.selected_photos) > 0:
                print(f"[GooglePhotosLayoutLegacy] ⌨️ DELETE - Delete {len(self.selected_photos)} photos")
                self._on_delete_selected()
                event.accept()
            else:
                super().keyPressEvent(event)
        ...
        # Ctrl+N: New project
            ...
            print("[GooglePhotosLayoutLegacy] ⌨️ Ctrl+N - New project")
        ...
        # Enter: Open first selected photo
            ...
            if len(self.selected_photos) > 0:
                ...
                print(f"[GooglePhotosLayoutLegacy] ⌨️ ENTER - Open {first_photo}")
        ...
        # Space: Quick preview (full screen)
            ...
            if len(self.selected_photos) > 0:
                first_photo = list(self.selected_photos)[0]
                print(f"[GooglePhotosLayoutLegacy] ⌨️ SPACE - Quick preview {first_photo}")
                self._on_photo_clicked(first_photo)
                event.accept()
            else:
                super().keyPressEvent(event)
        ...
        # S: Toggle selection mode
            ...
            print("[GooglePhotosLayoutLegacy] ⌨️ S - Toggle selection mode")
        ...
        # +/=: Zoom in
            ...
            print("[GooglePhotosLayoutLegacy] ⌨️ + - Zoom in")
        ...
        # -: Zoom out
            ...
            print("[GooglePhotosLayoutLegacy] ⌨️ - - Zoom out")
            if hasattr(self, 'zoom_slider'):
                current = self.zoom_slider.value()
        ...
        # G: Grid View
            ...
            print("[GooglePhotosLayoutLegacy] ⌨️ G - Grid view")
        ...
        # T: Timeline View
            ...
            print("[GooglePhotosLayoutLegacy] ⌨️ T - Timeline view")
        ...
        # E: Single View
            ...
            print("[GooglePhotosLayoutLegacy] ⌨️ E - Single view")
        ...
        # F: Toggle favorite for selected photos
            ...
            if len(self.selected_photos) > 0:
                print(f"[GooglePhotosLayoutLegacy] ⌨️ F - Toggle favorite for {len(self.selected_photos)} photos")
    ...
    def _toggle_selection_mode(self, checked: bool):
        ...
        print(f"[GooglePhotosLayoutLegacy] Selection mode: {'ON' if checked else 'OFF'}")
    ...
    def _handle_drag_select_release(self, pos):
        ...
        if selected_count > 0:
            print(f"[GooglePhotosLayoutLegacy] Drag-selected {selected_count} photos")
            self._update_selection_ui()
    ...
    def _on_batch_edit_location_clicked(self):
        ...
        print(f"[GooglePhotosLayoutLegacy] 📍 Batch location edit triggered from toolbar for {len(self.selected_photos)} photos")
    ...
    def _on_delete_selected(self):
        ...
        print(f"[GooglePhotosLayoutLegacy] Deleting {count} photos...")
    ...
    def _on_favorite_selected(self):
        ...
        try:
            ...
            if has_favorite:
                ...
                print(f"[GooglePhotosLayoutLegacy] Unfavorited {count} photos")
            else:
                # Favorite all
                for path in paths:
                    db.add_tag(path, "favorite", self.project_id)
                msg = f"⭐ Added {count} photo{'s' if count > 1 else ''} to favorites"
                print(f"[GooglePhotosLayoutLegacy] Favorited {count} photos")
            ...
            print(f"[GooglePhotosLayoutLegacy] ⚠️ Error toggling favorites: {e}")
            import traceback
            traceback.print_exc()
    ...
    def _on_semantic_search(self, photo_ids: list, query: str, scores: list):
        ...
        try:
            ...
            logger.info(f"[GooglePhotosLayoutLegacy] 🔍✨ Semantic search: {len(photo_ids)} results for '{query}'")
            ...
            if photo_data:
                ...
                logger.info(
                    f"[GooglePhotosLayoutLegacy] Semantic search displayed {len(photo_data)} results - "
            ...
            logger.error(f"[GooglePhotosLayoutLegacy] Semantic search failed: {e}", exc_info=True)
            from PySide6.QtWidgets import QMessageBox
            QMessageBox.critical(
                self.main_window,
    ...
    def _on_semantic_search_cleared(self):
        ...
        try:
            logger.info("[GooglePhotosLayoutLegacy] Semantic search cleared, reloading photos")
            self._load_photos()

        except Exception as e:
            logger.error(f"[GooglePhotosLayoutLegacy] Failed to clear semantic search: {e}", exc_info=True)
    ...
    def _on_smart_find_results(self, paths: list, query_label: str):
        ...
        try:
            logger.info(
                f"[GooglePhotosLayoutLegacy] Smart Find: '{query_label}' → {len(paths)} results"
            )
            ...
            if paths:
                ...
                self.timeline_layout.addWidget(empty_widget)
                logger.info(
                    f"[GooglePhotosLayoutLegacy] Smart Find empty state shown for '{query_label}'"
                )

        except Exception as e:
            logger.error(f"[GooglePhotosLayoutLegacy] Smart Find display failed: {e}", exc_info=True)

    def _on_smart_find_cleared(self):
        """Handle Smart Find cleared - restore full photo grid."""
        try:
            logger.info("[GooglePhotosLayoutLegacy] Smart Find cleared, reloading all photos")
            self._clear_filter()
        except Exception as e:
            logger.error(f"[GooglePhotosLayoutLegacy] Smart Find clear failed: {e}", exc_info=True)
    ...
    def _on_smart_find_scores(self, scores: object):
        ...
        try:
            ...
            logger.debug(
                f"[GooglePhotosLayoutLegacy] Received {len(self._smart_find_scores)} confidence scores"
            )
        except Exception as e:
            logger.error(f"[GooglePhotosLayoutLegacy] Smart Find scores failed: {e}")
            ...
            if hasattr(self, 'accordion_sidebar'):
                ...
                if find:
                    ...
                    if service:
                        ...
                        logger.info(f"[GooglePhotosLayoutLegacy] Excluded photo from results: {path}")
        except Exception as e:
            logger.error(f"[GooglePhotosLayoutLegacy] Smart Find exclude failed: {e}")
    ...
    def _filter_people_by_count(self, operator: str, threshold: int):
        ...
        try:
            ...
            print(f"[GooglePhotosLayoutLegacy] Failed to filter by count: {e}")
            import traceback
            traceback.print_exc()
    ...
    def _search_multi_person(self, person_names: list):
        ...
        try:
            ...
            print(f"[GooglePhotosLayoutLegacy] Multi-person search failed: {e}")
            import traceback
            traceback.print_exc()
    ...
    def _add_search_header(self, message: str):
        ...
        try:
            ...
            print(f"[GooglePhotosLayoutLegacy] Failed to add search header: {e}")
    ...
    def _on_people_search(self, text: str):
        ...
        try:
            ...
            print(f"[GooglePhotosLayoutLegacy] Autocomplete error: {e}")
            self.people_autocomplete.hide()
    ...
    def _rebuild_timeline_with_results(self, rows, search_text: str):
        ...
        if not rows:
            ...
            empty_label.setStyleSheet("font-size: 12pt; color: #888; padding: 60px;")
            self.timeline_layout.addWidget(empty_label)
            print(f"[GooglePhotosLayoutLegacy] No search results for: '{search_text}'")
            return
        ...
        print(f"[GooglePhotosLayoutLegacy] Search results: {len(rows)} photos in {len(photos_by_date)} dates")
    ...
    def _on_zoom_changed(self, value: int):
        ...
        """
        logger.info("[GooglePhotosLayoutLegacy] 🔎 Zoom changed to: %spx", value)
    ...
    def _commit_zoom_change(self):
        ...
        logger.info(
            "[GooglePhotosLayoutLegacy] Saving scroll position: %s/%s (%0.2f%%)",
            current_scroll,
            max_scroll,
    ...
    def _restore_scroll_percentage(self, percentage: float):
        ...
        if not timeline:
            logger.debug("[GooglePhotosLayoutLegacy] Timeline gone during scroll restore; skipping")
            return
        ...
        try:
            ...
            logger.debug(
                "[GooglePhotosLayoutLegacy] Timeline destroyed before scroll restore; skipping",
            )
            return
        ...
        logger.info(
            "[GooglePhotosLayoutLegacy] Restoring scroll position: %s/%s (%0.2f%%)",
            new_position,
            new_max,
            percentage * 100,
        )
    ...
    def _set_aspect_ratio(self, mode: str):
        ...
        """
        logger.info("[GooglePhotosLayoutLegacy] 📐 Aspect ratio changed to: %s", mode)
    ...
    def _clear_filter(self):
        ...
        """
        print("[GooglePhotosLayoutLegacy] Clearing all filters")
    ...
    def _on_detect_duplicates(self):
        ...
        try:
            ...
            logger.info("[GooglePhotosLayoutLegacy] Refreshing grid to show duplicate/similar badges...")
            self._load_photos()

        except Exception as e:
            print(f"[GooglePhotosLayoutLegacy] Error opening duplicate detection dialog: {e}")
            import traceback
            traceback.print_exc()
            from PySide6.QtWidgets import QMessageBox
            QMessageBox.critical(
    ...
    def _on_find_similar_photos(self):
        ...
        try:
            ...
            # Badges are only created when thumbnails are created, so we need to reload
            logger.info("[GooglePhotosLayoutLegacy] Refreshing grid to show similar stack badges...")
            self._load_photos()
            ...
            print(f"[GooglePhotosLayoutLegacy] ERROR: {error_msg}")
            QMessageBox.critical(
                self.main_window if hasattr(self, 'main_window') else None,
    ...
    def _on_show_duplicate_status(self):
        ...
        try:
            ...
            print(f"[GooglePhotosLayoutLegacy] Error getting duplicate status: {e}")
            import traceback
            traceback.print_exc()
            from PySide6.QtWidgets import QMessageBox
            QMessageBox.critical(
    ...
    def _get_photo_id_for_path(self, path: str) -> int:
        ...
        try:
            ...
            with db.get_connection() as conn:
                ...
                row = cursor.fetchone()
                if row:
                    print(f"[GooglePhotosLayoutLegacy] Found photo_id via case-insensitive match for: {path}")
                    return row['id']
                print(f"[GooglePhotosLayoutLegacy] ⚠️ No photo_metadata row found for path: {path}")
                return None
        except Exception as e:
            print(f"[GooglePhotosLayoutLegacy] ⚠️ Error getting photo ID for {path}: {e}")
            return None
    ...
    def _show_metadata_editor_for_photo(self, path: str):
        ...
        print(f"[GooglePhotosLayoutLegacy] Opening metadata editor for: {path}")
        main_window = getattr(self, 'main_window', None)
        if not main_window:
            print("[GooglePhotosLayoutLegacy] ⚠️ Cannot open metadata editor: main_window not found")
            return

        dock = getattr(main_window, 'metadata_editor_dock', None)
        if not dock:
            print("[GooglePhotosLayoutLegacy] ⚠️ Cannot open metadata editor: metadata_editor_dock not found")
            return

        photo_id = self._get_photo_id_for_path(path)
        if not photo_id:
            print(f"[GooglePhotosLayoutLegacy] ⚠️ Cannot open metadata editor: no photo_id found for path: {path}")
            return

        print(f"[GooglePhotosLayoutLegacy] ✓ Opening metadata editor for photo_id={photo_id}")
        main_window.show_metadata_for_photo(photo_id, path)
    ...
    def _on_duplicate_action_taken(self, action: str, asset_id: int):
        ...
        """
        print(f"[GooglePhotosLayoutLegacy] Duplicate action taken: {action} on asset {asset_id}")

        # Refresh the photo grid to remove deleted photos
        if action == "delete":
            print(f"[GooglePhotosLayoutLegacy] Refreshing grid after deletion...")
    ...
    def _on_stack_action_taken(self, action: str, stack_id: int):
        ...
        """
        print(f"[GooglePhotosLayoutLegacy] Stack action taken: {action} on stack {stack_id}")
    ...
    def _on_duplicate_badge_clicked(self, asset_id: int):
        ...
        try:
            ...
            print(f"[GooglePhotosLayoutLegacy] Opening duplicates dialog for asset {asset_id}")
    ...
    def _filter_favorites(self):
        ...
        try:
            ...
            print(f"[GooglePhotosLayoutLegacy] ⚠️ Error filtering favorites: {e}")
    ...
    def _show_single_view(self):
        ...
        try:
            ...
            print(f"[GooglePhotosLayoutLegacy] ⚠️ Error opening single view: {e}")
            
    def on_layout_activated(self):
        """Called when this layout becomes active."""
        print("[GooglePhotosLayoutLegacy] Layout activated")
    ...
    def _recheck_column_count(self):
        ...
        if old_cols is not None and new_cols != old_cols:
            print(f"[GooglePhotosLayoutLegacy] Column count changed {old_cols} -> {new_cols}, refreshing grid")
            self._load_photos(
                thumb_size=thumb_size,
    ...
    def on_layout_deactivated(self):
        ...
        """
        print("[GooglePhotosLayoutLegacy] 🧹 Layout deactivated - starting cleanup...")
        self.cleanup()
        print("[GooglePhotosLayoutLegacy] ✓ Cleanup complete")
    ...
    def cleanup(self):
        ...
        """
        print("[GooglePhotosLayoutLegacy] Cleaning up resources...")
    ...
    def _disconnect_all_signals(self):
        ...
        print("[GooglePhotosLayoutLegacy]   ↳ Disconnecting signals...")
        ...
        print("[GooglePhotosLayoutLegacy]   ✓ Signals disconnected")

    def _remove_event_filters(self):
        """Remove all event filters to prevent memory leaks."""
        print("[GooglePhotosLayoutLegacy]   ↳ Removing event filters...")
        ...
        print("[GooglePhotosLayoutLegacy]   ✓ Event filters removed")

    def _stop_all_timers(self):
        """Stop all QTimer instances to prevent crashes after widget deletion."""
        print("[GooglePhotosLayoutLegacy]   ↳ Stopping timers...")
        ...
        print("[GooglePhotosLayoutLegacy]   ✓ Timers stopped")

    def _cleanup_thread_pools(self):
        """Clean up thread pools to prevent background thread leaks."""
        print("[GooglePhotosLayoutLegacy]   ↳ Cleaning up thread pools...")
        ...
        print("[GooglePhotosLayoutLegacy]   ✓ Thread pools cleaned")

    def _clear_caches(self):
        """Clear all caches to prevent unbounded memory growth."""
        print("[GooglePhotosLayoutLegacy]   ↳ Clearing caches...")
        ...
        print("[GooglePhotosLayoutLegacy]   ✓ Caches cleared")

    def _stop_animations(self):
        """Stop all animations in child widgets (CollapsibleSection)."""
        print("[GooglePhotosLayoutLegacy]   ↳ Stopping animations...")
        ...
        print("[GooglePhotosLayoutLegacy]   ✓ Animations stopped")

    def _on_create_project_clicked(self):
        """Handle Create Project button click."""
        print("[GooglePhotosLayoutLegacy] 🆕🆕🆕 CREATE PROJECT BUTTON CLICKED! 🆕🆕🆕")

        # Debug: Check if main_window exists and has breadcrumb_nav
        if not hasattr(self, 'main_window'):
            print("[GooglePhotosLayoutLegacy] ❌ ERROR: self.main_window does not exist!")
            return
        ...
        if not hasattr(self.main_window, 'breadcrumb_nav'):
            print(f"[GooglePhotosLayoutLegacy] ❌ ERROR: main_window does not have breadcrumb_nav!")
            return

        if not hasattr(self.main_window.breadcrumb_nav, '_create_new_project'):
            print(f"[GooglePhotosLayoutLegacy] ❌ ERROR: breadcrumb_nav does not have _create_new_project method!")
            return

        print("[GooglePhotosLayoutLegacy] ✓ Calling breadcrumb_nav._create_new_project()...")

        # Call BreadcrumbNavigation's project creation dialog
        self.main_window.breadcrumb_nav._create_new_project()

        print("[GooglePhotosLayoutLegacy] ✓ Project creation dialog completed")

        # CRITICAL: Update project_id after creation
        from app_services import get_default_project_id
        new_project_id = get_default_project_id()
        print(f"[GooglePhotosLayoutLegacy] Updated project_id: {new_project_id}")
        ...
        print("[GooglePhotosLayoutLegacy] ✓ Layout refreshed after project creation")
    ...
    def _populate_project_selector(self):
        ...
        try:
            ...
            print(f"[GooglePhotosLayoutLegacy] Project selector populated with {len(projects)} projects (+ New Project option)")

        except Exception as e:
            print(f"[GooglePhotosLayoutLegacy] ⚠️ Error populating project selector: {e}")
    ...
    def _execute_debounced_reload(self):
        ...
        if signature == self._last_reload_signature:
            print(f"[GooglePhotosLayoutLegacy] Skipping duplicate reload: {signature}")
            return

        if not getattr(self, 'project_id', None):
            print("[GooglePhotosLayoutLegacy] Suppressing reload — no active project")
            return

        self._reload_in_progress = True
        try:
            self._last_reload_signature = signature
            print(f"[GooglePhotosLayoutLegacy] Executing debounced reload: {signature[0] if signature else 'unknown'}")
    ...
    def _on_project_changed(self, index: int):
        ...
        if new_project_id == "__new_project__":
            print("[GooglePhotosLayoutLegacy] ➕ New Project option selected")
    ...
    def set_project(self, project_id):
        ...
        if project_id is not None and project_id == self.project_id:
            print(f"[GooglePhotosLayoutLegacy] set_project() called: already on project {project_id}, skipping")
            return

        print(f"[GooglePhotosLayoutLegacy] set_project() called: {self.project_id} -> {project_id}")
        ...
        try:
            ...
            if hasattr(self, 'accordion_sidebar') and self.accordion_sidebar is not None:
                try:
                    ...
                    print(f"[GooglePhotosLayoutLegacy] Accordion project sync failed: {e}")
            ...
            if project_id is None:
                print("[GooglePhotosLayoutLegacy] ⚠️ No project selected — skip loading")
                return
    ...
    def refresh_after_scan(self) -> None:
        ...
        if self.project_id is None:
            ...
            if self.project_id is not None:
                print(f"[GooglePhotosLayoutLegacy] project_id recovered: {self.project_id}")
                sidebar = getattr(self, 'sidebar', None)
                if sidebar and hasattr(sidebar, 'set_project'):
                    sidebar.set_project(self.project_id)

        if not getattr(self, 'project_id', None):
            print("[GooglePhotosLayoutLegacy] refresh_after_scan: no project — suppressing load")
            return
    ...
    def _toggle_favorite_single(self, path: str):
        ...
        try:
            ...
            if is_favorited:
                ...
                print(f"[GooglePhotosLayoutLegacy] Unfavorited: {os.path.basename(path)}")
            else:
                tag_service.assign_tags_bulk([path], "favorite", self.project_id)
                msg = f"⭐ Added to favorites: {os.path.basename(path)}"
                print(f"[GooglePhotosLayoutLegacy] Favorited: {os.path.basename(path)}")
            ...
            print(f"[GooglePhotosLayoutLegacy] ⚠️ Error toggling favorite: {e}")
            import traceback
            traceback.print_exc()
        ...
        if ok and tag_name.strip():
            try:
                ...
                if count > 0:
                    ...
                    print(f"[GooglePhotosLayoutLegacy] {msg}")
                    ...
                    print(f"[GooglePhotosLayoutLegacy] {error_msg}")
                    from PySide6.QtWidgets import QMessageBox
                    QMessageBox.warning(
                        self.main_window,
                        "Tag Failed",
                ...
                print(f"[GooglePhotosLayoutLegacy] ⚠️ Error adding tag: {e}")
                import traceback
                traceback.print_exc()
                from PySide6.QtWidgets import QMessageBox
                QMessageBox.critical(
    ...
    def _refresh_tag_overlays(self, paths: List[str]):
        ...
        try:
            ...
            print(f"[GooglePhotosLayoutLegacy] ✓ Refreshed tag badges for {len(paths)} photos")
            
            # Also refresh Favorites sidebar section
            try:
                self._build_tags_tree()
            except Exception as e:
                print(f"[GooglePhotosLayoutLegacy] ⚠️ Could not refresh tags section: {e}")
        
        except Exception as e:
            print(f"[GooglePhotosLayoutLegacy] ⚠️ Error refreshing tag overlays: {e}")
            import traceback
            traceback.print_exc()
